...
def main():
    parser = argparse.ArgumentParser(description="Landsat Analysis Ready Data (GLAD ARD)")
    ...
    parser.add_argument("--start_month",
                        help="start month in specific year based on the code e.g. 24-May 2000 > 838 (int) ", type=int)
    parser.add_argument("--end_month", help="end month in specific year based on code e.g. 12-Sep 2000 > 844 (int) ",
                        type=int)
    parser.add_argument("--count_years", help="how many years would like to download (int) ", type=int)
    ...
    for t in range(len(tiles)):
        for k in times:
            ...
            if not os.path.exists(out_path / tiles[t] / (str(k) + ".tif")):
                r = requests.get(url.format(lat=tiles[t][-3:], tile=tiles[t], interval=k), auth=(username, password))
                print(r.url)

                print(output.format(p=out_path / tiles[t], interval=k))
                open(output.format(p=out_path / tiles[t], interval=k), 'wb').write(r.content)
